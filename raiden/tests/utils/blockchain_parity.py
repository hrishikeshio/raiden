...
DEFAULT_PASSPHRASE = 'notsosecret'  # parity's account passphrase
DAGSIZE = 1073739912
...
def parity_to_cmd(node, datadir, verbosity):
    """
    ...
    log.debug('parity command: {}'.format(cmd))

    return cmd


def parity_create_account(datadir, privkey):
    """
    ...
    create = subprocess.Popen(
        ['parity', '--base-path', datadir, 'account', 'import', keyfile_path],
...
def parity_bare_genesis(genesis_path, private_keys):
...
def parity_init_datadir(datadir, genesis_path):
    ...
    """
    try:
        subprocess.check_output(['parity', '--base-path', datadir, 'init', genesis_path], stderr=subprocess.STDOUT)
    except subprocess.CalledProcessError as e:
        raise ValueError(
            """Initializing parity with custom genesis returned {} with error:
{}""".format(e.returncode, e.output))


def parity_wait_and_check(privatekeys, rpc_ports):
    """ Wait until the parity cluster is ready. """
    ...
    if jsonrpc_running is False:
        raise ValueError('parity didnt start the jsonrpc interface')
...
def parity_create_blockchain(
    ...
    if memory.free < DAGSIZE:
        raise RuntimeError(
            'To properly run the tests with a parity miner at least 1GB of free memory is required.'
        )
    ...
    for i, config in enumerate(nodes_configuration):
        ...
        assert len(nodedir + '/geth.ipc') < 108, 'parity data path is too large'

        os.makedirs(nodedir)

        if genesis_path is None:
            parity_bare_genesis(node_genesis_path, all_keys)
        else:
            shutil.copy(genesis_path, node_genesis_path)

        parity_init_datadir(nodedir, node_genesis_path)

        if 'minerthreads' in config:
            parity_create_account(nodedir, private_keys[i])

        commandline = parity_to_cmd(config, nodedir, verbosity)
        cmds.append(commandline)

    # save current term settings before running parity
    ...
    parity_wait_and_check(private_keys, rpc_ports)

    # reenter echo mode (disabled by parity pasphrase prompt)
